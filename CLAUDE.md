...
跨平台输入法，Core 平台无关，各平台只做壳。已发版 0.1.3（macOS 与 Windows 共用版本号，2026-09-17）；Linux 上游第一阶段提供 Rust Server + Fcitx5 默认候选面板（源码安装，尚未发版），本 fork 另有 GNOME+Wayland+ibus 壳 MVP（`apps/linux`），见 `docs/design/ibus-qingjian.md`。
...
- `apps/linux`：两条 Linux 路线并存。上游的 Fcitx5 栈：`server`（Engine + Unix socket）+ `fcitx5`（框架事件、默认面板）+ `scripts/install.sh`（用户级安装卸载）；Server 手动启动，见 `docs/notes/linux-fcitx5.md`。
  本 fork 的 ibus 壳：crate `qingjian-linux`（`src/`，二进制 `ibus-engine-qingjian`）+ `scripts/install-dev.sh` + `packaging/`，见 `docs/design/ibus-qingjian.md`。
