...
- 配置项：云联想 `[predict]`（偏好设置「云服务」页有「测试连接」按钮：`qingjian_predict::ConnectionTest` 起线程发一条最小请求，`Host` 用独立定时器 `CloudTestMonitor` 轮询结果显示到窗口底部；
  `reasoning_effort` 缺省 `none`，DeepSeek V4 默认思考，不关正文为空）；模糊音 `[fuzzy]` 默认都关；`[general]` 学习语言（`off` 不显示译文）/ 每页候选数 / 翻页键 / 外观 / 竖排横排 / 拼音显示位置 / 拼音行显示原始键 `raw_preedit`（双拼 `nihc` 显示 `nihc` 不解成 `ni'hao`，只影响显示；2026-09-21 起有配置，目前仅 Linux 壳消费）/
...
- 配置覆盖面：`traditional` / `english_candidates` / `log_level`（启动时读）与 `[dictionaries]`（附加词库 / 领域词库开关）都已接；
  ...
  主题 / 外观各项对 ibus 原生候选窗无意义（观感随系统）；`[general] preedit` 模式
  （both / inline / window）在 Linux 壳也仍未接——配置里写了不生效，内联 preedit 与辅助行
  目前总是同发（gnome-shell 面板与终端 / XIM 各取所需）。
- `[general] raw_preedit`（2026-09-21，拼音行显示原始键）：开了显示敲的键（`composition().typed_text()` + 字节光标，
  双拼 `nihc` 不再解成 `ni'hao`，中文模式下 Shift 敲的大写还原）——`present::preedit_of` 一处分流，
  内联 preedit 与辅助行同变；只影响显示，查询、学习、上屏消耗换算（`Decoded::keys_for`）都按解出的全拼走。
  目前只有 Linux 壳消费，mac / Windows 壳还没接。
